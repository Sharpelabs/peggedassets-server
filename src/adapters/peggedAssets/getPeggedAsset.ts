...
import { normalizeChain, normalizedChainReplacements } from "../../utils/normalizeChain";
...
// (async () => {
  ...
  let passedFile = `/home/ubuntu/peggedassets-server/src/adapters/peggedAssets/${adapterName}`;
  try {
  ...
  const assetCirculating: { [key: string]: { [key: string]: number } } = {};

  // console.log(normalizedChainReplacements);

  Object.entries(peggedBalances).forEach(([chain, issuances]) => {
    if (chain !== "totalCirculating") {
      Object.entries(normalizedChainReplacements).forEach(([key, value]) => {
        chain = chain === value ? decodeURIComponent(key) : chain;
      });
      assetCirculating[chain] = {};
      // console.log(`--- ${chain} ---`);
      Object.entries(issuances)
        .sort((a, b) => (b[1][pegType] ?? 0) - (a[1][pegType] ?? 0))
        .forEach(([issuanceType, issuance]) => {
          assetCirculating[chain][issuanceType] = issuance[pegType] ?? 0;
          // console.log(
  ...
  await couchClient.insert(`defi-stablecoins/${adapterName}`, assetCirculating);
  // process.exit(0);
// })();
}
...
export async function getAllAssets() {
  const directoryPath = `/home/ubuntu/peggedassets-server/src/adapters/peggedAssets/`;
  const SC_IDS = await couchClient.retrieve("cache/stablecoins-id");
  ...
  for (let i = 0; i < files.length; i++) {
    // if (files[i] !== "dai") continue;
    try {
        console.log(i, files[i]);
        let pegType = SC_IDS[files[i]]["pegType"];
        await getAsset(files[i], pegType);
    } catch (e) {
        // console.log(e);
        continue;
    }
